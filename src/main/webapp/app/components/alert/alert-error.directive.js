(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('wilkestechDemoApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-wilkestechDemoApp-error');
                var entityKey = httpResponse.headers('X-wilkestechDemoApp-params');
                if (errorHeader) {
